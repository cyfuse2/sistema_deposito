...
class SistemaGerenciamentoDeposito:
    ...
    def criar_banco_empresa(self, db_nome):
        conn = sqlite3.connect(f'deposito_empresas/{db_nome}.db')
        cursor = conn.cursor()
    ...
    def obter_conexao_empresa(self):
        if self.empresa_logada:
            return sqlite3.connect(f'deposito_empresas/{self.empresa_logada["db_nome"]}.db')
        return None
    ...
    def cadastrar_empresa(self):
        ...
        try:
            ...
            cursor.execute("INSERT INTO empresas (nome, senha, logo_path, db_nome) VALUES (?, ?, ?, ?)", 
                         (nome, senha_hash, logo_final, db_nome))
    ...
    def fazer_login(self):
        ...
        try:
            ...
            if empresa := cursor.fetchone():
                db_path = f'deposito_empresas/{empresa[4]}.db'
    ...
    def tela_cadastro_produto(self):
        ...
        ttk.Button(container, text="Salvar Produto", command=self.salvar_produto, style='TButton').pack(pady=20)
    ...
    def tela_relatorio_estoque(self):
        ...
        try:
            ...
            cursor.execute("SELECT nome, quantidade FROM produtos WHERE quantidade < 10 ORDER BY quantidade ASC")
